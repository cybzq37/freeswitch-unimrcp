...
MRCP_PLUGIN_DECLARE(mrcp_engine_t*) mrcp_plugin_create(apr_pool_t *pool)
{
	apt_log(RECOG_LOG_MARK, APT_PRIO_INFO, "==========mrcp_plugin_create ...");
...
static apt_bool_t demo_recog_engine_open(mrcp_engine_t *engine)
{
	apt_log(RECOG_LOG_MARK, APT_PRIO_INFO, "==========demo_recog_engine_open ...");
	demo_recog_engine_t *demo_engine = engine->obj;
	if(demo_engine->task) {
		apt_task_t *task = apt_consumer_task_base_get(demo_engine->task);
		apt_task_start(task);
...
static mrcp_engine_channel_t* demo_recog_engine_channel_create(mrcp_engine_t *engine, apr_pool_t *pool)
{
    ...
    //>>>
	recog_channel->audio_filepath = NULL;
	recog_channel->ars_url = apr_table_get(config->params, "ARS_URL");
...
static apt_bool_t demo_recog_channel_recognize(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response)
{
	...
	if(!recog_channel->audio_out) {
		...
		recog_channel->audio_filepath = apt_vardir_filepath_get(dir_layout,file_name,channel->pool);
		if(recog_channel->audio_filepath) {
			apt_log(RECOG_LOG_MARK,APT_PRIO_INFO,"Open Utterance Output File [%s] for Writing", recog_channel->audio_filepath);
			recog_channel->audio_out = fopen(recog_channel->audio_filepath,"wb+");
			if(!recog_channel->audio_out) {
				apt_log(RECOG_LOG_MARK,APT_PRIO_WARNING,"Failed to Open Utterance Output File [%s] for Writing", recog_channel->audio_filepath);
			}
		}
	}
	...
	recog_channel->last_result = NULL;
	// ˢ�»�����
	fflush(recog_channel->audio_out);
	// ����ļ�����
	ftruncate(fileno(recog_channel->audio_out), 0);
	// ����ָ��
	fseek(recog_channel->audio_out, 0, SEEK_SET);
	return TRUE;
}
...
{
	apt_log(RECOG_LOG_MARK, APT_PRIO_INFO, "==========demo_recog_result_load ...");
	//>>>>>
	// û�з��ؽ��������false
	...
	// ������λ������������ͬ apt_string_assign_n(&message->body, text, size, message->pool);
	// ����һ��recog��request��ƴ��һ��response
	apt_str_t* body = &message->body;
	body->buf = apr_psprintf(message->pool, "%s", recog_channel->last_result);
	if (body->buf) {
...
static size_t write_callback(char* ptr, size_t size, size_t nmemb, void* userdata)
{
	...
	*response_data = realloc(*response_data, data_len + 1);
	if (*response_data == NULL) {
		fprintf(stderr, "Failed to allocate memory for response data\n");
		return 0;
	}
	memcpy(*response_data, ptr, data_len);
...
{
	apt_log(RECOG_LOG_MARK, APT_PRIO_INFO, "==========demo_recog_recognition_complete...");
	char*	input_buf;
	int		input_size;
	
	// ��ȡ�ļ���С
	fseek(recog_channel->audio_out, 0, SEEK_END);
	input_size = ftell(recog_channel->audio_out);
	fseek(recog_channel->audio_out, 0, SEEK_SET);
	input_buf = (char*)malloc(input_size + 1);
	if (input_buf == NULL) {
		return FALSE;
	}
	size_t read_size = fread(input_buf, 1, input_size, recog_channel->audio_out);
	...
	if (curl) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, recog_channel->ars_url);
		// �ļ���Ϣ
		curl_easy_setopt(curl, CURLOPT_VERBOSE, 1L);
		curl_easy_setopt(curl, CURLOPT_POSTFIELDS, input_buf);		// POST ����������
		curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, input_size);	// POST�����ݳ��� 

		// �ص���Ϣ
		long	response_code;
		char*	response_data = NULL;
		double	response_len = 0.0;
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_callback);  //���ûص�����
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, &response_data);        //����д����
		...
		if (response_len > 0) {
			...
			if (NULL == recog_channel->last_result) {
				recog_channel->last_result = apr_pstrdup(recog_channel->channel->pool, response_data);
			}
			else {
				recog_channel->last_result = apr_pstrcat(recog_channel->channel->pool, recog_channel->last_result, response_data, NULL);
			}
		}
	...
	switch(demo_msg->type) {
		...
		case DEMO_RECOG_MSG_CLOSE_CHANNEL:
		{
			...
			if(recog_channel->audio_out) {
				...
				remove(recog_channel->audio_filepath);
				recog_channel->audio_out = NULL;
				recog_channel->audio_filepath = NULL;
			}
